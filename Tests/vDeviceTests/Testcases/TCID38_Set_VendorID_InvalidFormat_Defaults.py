"""
/**
 * @file TCID38_Set_VendorID_InvalidFormat_Defaults.py
 * @brief Validate invalid vendor-id format is handled in plugin and defaults are applied.
 */
"""
...
def _normalize_vendor(v):
    if not isinstance(v, str):
        return ""
    # Plugin may expose vendor id as "019fb" or "00:19:FB" depending on stack formatting.
...
def run_test():
    ...
    set_resp = send_curl_command(HdmiCecSourceApis.set_vendor_id_invalid_format)
    if not set_resp:
        log_error("✖ setVendorId(invalid format) command not sent")
        return False
    log_warning(f"setVendorId(invalid format) response: {set_resp}")

    set_body = _parse_json(set_resp)
    if set_body is None:
        log_error("TCID38 failed: invalid JSON response")
        return False

    if _is_framework_validation_error(set_body):
        log_error("TCID38 failed: request rejected before plugin SetVendorId catch block")
        return False
    ...
    if not isinstance(get_body, dict):
        log_error("TCID38 failed: getVendorId invalid JSON")
        return False

    vendor = get_body.get("result", {}).get("vendorid", "")
    if _normalize_vendor(vendor) == EXPECTED_DEFAULT_VENDOR:
        elapsed = time.perf_counter() - start_time
        msg = "TCID38_Set_VendorID_InvalidFormat_Defaults Passed ✅"
    ...
    log_error("TCID38_Set_VendorID_InvalidFormat_Defaults Failed ❌")
    return False
