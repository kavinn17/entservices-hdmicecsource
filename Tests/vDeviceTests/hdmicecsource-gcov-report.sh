...
OUT_DIR="${1:-/tmp/hdmicecsource-coverage}"
GCDA_DIR="/tmp/gcov"
GCNO_DIRS="/usr/lib/gcov/entservices-hdmicecsource /usr/lib/gcov/hdmicec /usr/lib/gcov/hdmicecsource"
...
has_component_gcda() {
    find "${GCDA_DIR}" -type f -name '*.gcda' | grep -Eq '/entservices-hdmicecsource/|/hdmicec/'
}
...
if ! has_component_gcda; then
    echo "No HdmiCecSource/CCEC gcda files found under ${GCDA_DIR}." >&2
    exit 1
fi

PLUGIN_GCDA_COUNT="$(count_gcda entservices-hdmicecsource)"
CCEC_GCDA_COUNT="$(count_gcda hdmicec)"
echo "Detected gcda counts: plugin=${PLUGIN_GCDA_COUNT}, ccec=${CCEC_GCDA_COUNT}"
...
find "${GCDA_DIR}" -type f \( -name '*.gcda' -o -name '*.gcno' \) | while IFS= read -r f; do
    case "${f}" in
        *"/entservices-hdmicecsource/"*|*"/hdmicec/"*)
            rel="${f#${GCDA_DIR}/}"
...
if ! find "${WORK_DIR}" -type f -name '*.gcda' | grep -q .; then
    echo "No HdmiCecSource/CCEC gcda files found under ${GCDA_DIR}. Run tests first." >&2
    exit 1
fi
...
# Filter to plugin + CCEC source files
...
lcov --extract "${OUT_DIR}/combined.info" \
    '*/entservices-hdmicecsource/*/git/plugin/*' \
    '*/entservices-hdmicecsource/*/git/ccec/*' \
    '*/hdmicec/*/git/ccec/*' \
...
if ! grep -q '^SF:' "${TOTAL_INFO}"; then
    echo "No plugin/CCEC coverage records found in ${OUT_DIR}/combined.info." >&2
    exit 1
fi

# Remap SF paths to local sources when debug sources are available in target.
HTML_INFO="${TOTAL_INFO}"
SOURCE_ROOT="$(find /usr/src/debug -type d -path '*/entservices-hdmicecsource/*/git' 2>/dev/null | head -n 1)"
SF_PREFIX="$(grep '^SF:' "${TOTAL_INFO}" | head -n 1 | sed -e 's#^SF:##' -e 's#/plugin/.*##' -e 's#/ccec/.*##')"
...
# Normalize SF paths so genhtml shows clean component names: Plugin and CCEC.
...
sed \
    -e 's|SF:.*/recipe-sysroot/.*/ccec/include/ccec/|SF:CCEC/|g' \
    -e 's|SF:.*/ccec/|SF:CCEC/|g' \
...
if command -v gcovr >/dev/null 2>&1; then
    ...
    if ! gcovr \
        ...
        --root "${GCOVR_ROOT}" \
        --filter '.*/entservices-hdmicecsource/.*/git/plugin/.*' \
        --filter '.*/entservices-hdmicecsource/.*/git/ccec/.*' \
        --filter '.*/hdmicec/.*/git/ccec/.*' \
        --xml-pretty \
...
echo ""
echo "Coverage info (plugin + ccec): ${TOTAL_INFO}"
echo ""
...
# Print per-component summaries for quick tracking.
lcov --extract "${TOTAL_INFO}" '*/entservices-hdmicecsource/*/git/plugin/*' \
...
lcov --extract "${TOTAL_INFO}" \
    '*/entservices-hdmicecsource/*/git/ccec/*' \
    '*/hdmicec/*/git/ccec/*' \
    --output-file "${CCEC_INFO}" --ignore-errors source || true
if ! grep -q '^SF:' "${CCEC_INFO}"; then
    lcov --extract "${TOTAL_INFO}" '*/ccec/*' \
        --output-file "${CCEC_INFO}" --ignore-errors source || true
fi
...
if grep -q '^SF:' "${CCEC_INFO}"; then
    echo "- CCEC"
    lcov --summary "${CCEC_INFO}"
else
    echo "- CCEC: no data"
    if [ "${CCEC_GCDA_COUNT}" -eq 0 ]; then
        echo "  reason: no /hdmicec/ *.gcda files were produced at runtime, so no CCEC row can appear in HTML." >&2
    fi
fi
